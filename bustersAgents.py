...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        # most likely pos for each living ghost
        most_likely_pos = []
        for beliefs in livingGhostPositionDistributions:
            # pos w highest probability
            max_prob = -1
            likely_pos = None
            for pos in beliefs:
                if beliefs[pos] > max_prob:
                    ...
                    likely_pos = pos
            most_likely_pos.append(likely_pos)
        
        # best action and min dist initialization
        best_action = None
        min_dist = 999999
        
        for action in legal:
            # successor position after action
            suc_pos = Actions.getSuccessor(pacmanPosition, action)
            
            # min maze dist to ghosts most likely pos
            for ghost_pos in most_likely_pos:
                # maze dist from suc pos to ghost pos
                dist = self.distancer.getDistance(suc_pos, ghost_pos)
                # update best action
                if dist < min_dist:
                    min_dist = dist
                    best_action = action
